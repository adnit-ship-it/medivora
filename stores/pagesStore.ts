...
export const usePagesStore = defineStore('pages', () => {
  ...
  // Get common config from pages.json
  const getCommonConfig = () => {
    return pages.value?.common || null
  }

  // Get icon registry from pages.json (returns Record<string, string> mapping icon type to path)
  const getIconRegistry = (): Record<string, string> => {
    if (!pages.value?.iconRegistry) return {}
    
    const registry: Record<string, string> = {}
    Object.entries(pages.value.iconRegistry).forEach(([key, value]: [string, any]) => {
      if (value?.path) {
        registry[key] = value.path
      }
    })
    return registry
  }
