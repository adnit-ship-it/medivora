    ...
    <div  class="w-full flex justify-center">
        <div class="w-full flex flex-col py-20 pt-0 md:py-24 px-6 lg:px-0 max-w-[1056px]">
            ...
            <div class="mt-10">
                ...
                >
                    {{ sectionData?.heading?.show ? sectionData?.heading?.text : ''  }}
                    <span v-if="props.sectionData?.paragraph?.show === true" :class="paragraphColorClass">
                            {{ props.sectionData?.paragraph?.text ?? ''  }}
                        </span>
                </h3>
            </div>
            <div
...
const shouldShowIntakeForm = computed(() => props.sectionData?.heading?.show !== false);
const defaultFields = [
...
const headingColorClass = computed(() => {
  ...
  // Return class if it's a Tailwind class (doesn't start with #)
  return resolved && !resolved.startsWith('#') ? resolved : '';
});



// Paragraph color - same pattern
const paragraphColorClass = computed(() => {
  const color = props.sectionData?.paragraph?.color;
  if (!color) return '';
  const resolved = resolveColorToken(color, 'text');
  return resolved && !resolved.startsWith('#') ? resolved : '';
});
...
// Note: Fields are in rawComponents - need to extract from there
const intakeFields = computed(() => {
    // Try to find fields in rawComponents
    const rawComponents = props.sectionData?.rawComponents || [];
    const fieldsComponent = rawComponents.find((comp: any) => comp.fields);
    const fields = fieldsComponent?.fields || [];
    const sourceFields = fields.length > 0 ? fields : defaultFields;
    return sourceFields.map((field: any, index: number) => ({
        label: field.label || `Field ${index + 1}`,
        placeholder: field.placeholder || "",
    }));
});
const intakeHighlightLines = computed(() => {
    console.log('props.sectionData?.paragraph?.text', props.sectionData?.paragraph?.text);
    return props.sectionData?.paragraph?.text   ;
});
