  ...
  <UiSectionWrapper v-if="shouldShowStatistics" class="bg-accentColor2 py-20">
    <UiSectionContainer class="flex flex-col gap-12">
      <h2 v-motion :initial="{ opacity: 0, y: 32 }" :visible-once="{
        ...
        class="w-full max-w-[1168px] font-defaultSerif text-[20px] md:text-[28px] lg:text-[32px] font-semibold md:text-left">
        {{ sectionData?.heading?.text || 'Clinically Proven To Help You Lose Weight' }}
      </h2>
      <div class="flex gap-12">
        ...
        }" :src="sectionData?.media?.image?.src || '/assets/images/wl-graph.png'" :alt="sectionData?.media?.image?.alt || 'weight loss graph'" class="hidden lg:block h-[388px] object-contain">
      </div>
    </UiSectionContainer>
  </UiSectionWrapper>
</template>
...
const shouldShowStatistics = computed(() => props.sectionData?.heading?.show !== false);
...
const statisticsCards = computed(() => {
  const mappedCards = (props.sectionData?.stats?.cards || []).map((card: any) => {
