    ...
    <UiSectionContainer class="flex-col">
      ...
      <div ref="sectionRef">
        ...
        <h2
          ...
          class="font-defaultSerif w-full text-[20px] md:text-[28px] lg:text-[32px] mb-9 md:mb-12 font-semibold"
        >
        ...
        <div class="flex gap-8 px-8 lg:px-0 max-w-[1168px] w-full">
          <div class="flex flex-col items-center lg:hidden py-10 relative">
            ...
            ></div>
            <div class="h-2 w-2 md:h-3 md:w-3 rounded-full" :style="{ backgroundColor: progressDotsColor }" />
            <div
            ...
            <div
              ...
              :style="progress >= (dotThresholds?.[1] ?? 0) ? { backgroundColor: progressDotsColor } : {}"
            />
            <div
              class="h-[108px] md:h-[152px] w-[1px] md:w-[2px] bg-[#DBD9D9]"
            ></div>
            <div
              ...
              :style="progress >= (dotThresholds?.[2] ?? 0) ? { backgroundColor: progressDotsColor } : {}"
            />
            <div
              class="h-[108px] md:h-[152px] w-[1px] md:w-[2px] bg-[#DBD9D9]"
            ></div>
            <div
              ...
              :style="progress >= (dotThresholds?.[3] ?? 0) ? { backgroundColor: progressDotsColor } : {}"
            />
            <div
              class="h-[108px] md:h-[152px] w-[1px] md:w-[2px] bg-[#DBD9D9]"
            ></div>
            <div
              ...
              :style="progress >= (dotThresholds?.[4] ?? 0) ? { backgroundColor: progressDotsColor } : {}"
            />
          </div>
          <div
...
// Progress line color from sectionData.media.progressLine.color
const progressLineColor = computed(() => {
  const color = props.sectionData?.media?.progressLine?.color;
  return color ? resolveIconColor(color) : "#A75809";
});

// Progress dots color from sectionData.media.progressDots.color
const progressDotsColor = computed(() => {
  const color = props.sectionData?.media?.progressDots?.color;
  return color ? resolveIconColor(color) : progressLineColor.value;
});
...
// Load and colorize the SVG for desktop background
const loadAndColorizeSvg = async () => {
  if (!process.client) return;
  
  try {
    const response = await fetch(defaultProgressLine);
    if (response.ok) {
      let svg = await response.text();
      const color = progressLineColor.value;
      
      // Replace hardcoded colors with dynamic color
      svg = svg.replace(/#A75809/g, color);
      
      // Convert to data URL for use as background image
      const base64 = btoa(unescape(encodeURIComponent(svg)));
      svgContent.value = `data:image/svg+xml;base64,${base64}`;
    }
  } catch (error) {
    console.error('Failed to load progress line SVG:', error);
    // Fallback to original SVG
    svgContent.value = defaultProgressLine;
  }
};

// Watch for color changes and reload SVG
watch(progressLineColor, () => {
  loadAndColorizeSvg();
}, { immediate: true });

const journeyBackgroundVars = computed(() => {
  const progressLine = svgContent.value || defaultProgressLine;
...
watch(
  ...
  (newSectionData) => {
    const steps =
      newSectionData?.steps && newSectionData.steps.length > 0
        ? newSectionData.steps
        : defaultJourneySteps;
    journeyCards.value = steps.map((step, index) => ({
      img: step.icon?.src || "",
      imgAlt: step.icon?.alt || step.title,
      title: step.title,
      subtext: formatSubtext(step.subtext ? [step.subtext] : []),
      isActive: index === 0,
