...
import { extractIconColors } from '../utils/colorTokens'
 ...
 * Merges media objects instead of overwriting to preserve all media properties
 */
function extractMedia(component: any, existingMedia?: any): Partial<ExtractedSectionData> {
  const extracted: Partial<ExtractedSectionData> = {}
  
  if (component.media) {
    // Merge with existing media instead of overwriting
    extracted.media = existingMedia 
      ? { ...existingMedia, ...component.media }
      : { ...component.media }
  }
...
function extractConfigs(component: any): Partial<ExtractedSectionData> {
  ...
  if (Array.isArray(component.reviews)) {
    extracted.reviews = component.reviews.map((review: any) => ({
      name: review.name || '',
...
function extractBackgroundColor(components: any[]): string | undefined {
  // Look for section-level background color (not from buttons)
  // Currently, sections.json doesn't have section-level backgroundColor,
  // so we return undefined to use the default (backgroundColor)
  
  // If in the future sections.json has a section-level backgroundColor field,
  // extract it here
  
  // Don't extract from button configs - those are for buttons only
  return undefined
}
...
export function extractSectionData(
  ...
  for (const component of sectionData.components) {
    ...
    // Extract media - merge with existing media to preserve all properties from multiple components
    const mediaExtracted = extractMedia(component, extracted.media)
    Object.assign(extracted, mediaExtracted)
  ...
  if (isDev && sectionData.name === 'Home CTA') {
    console.log('[sectionDataExtractor] Extracted data for Home CTA:', {
