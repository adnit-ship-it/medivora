<template>
  <div v-if="shouldShowContact" class="mx-auto bg-backgroundColor min-h-[98vh] py-20 md:py-32 flex flex-col items-center px-8 md:px-0">
    <template v-for="section in contactSections" :key="section?.name">
      <DynamicSection
    ...
    <div v-motion :initial="{ opacity: 0, y: 32 }" :visible-once="{
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col gap-6 px-6">
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ formConfig.labels.name }}</label>
          <input v-model="formData.name" :placeholder="formConfig.placeholders.name" type="text" :class="[
        ...
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ formConfig.labels.age }}</label>
          <input v-model="formData.age" :placeholder="formConfig.placeholders.age" type="number" min="1" max="120" :class="[
            'w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-accentColor1',
        ...
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ formConfig.labels.email }}</label>
          <input v-model="formData.email" :placeholder="formConfig.placeholders.email" type="email" :class="[
            'w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-accentColor1',
        ...
        <div>
          <label class="block text-lg md:text-xl text-black mb-1">{{ formConfig.labels.message }}</label>
          <textarea v-model="formData.message" :placeholder="formConfig.placeholders.message" rows="4" :class="[
            'w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-accentColor1',
...
const contactConfig = computed(() => pagesStore.getPageConfig('contact'));
const shouldShowContact = computed(() => (contactConfig.value?.show ?? true));

// Get sections for contact page
const contactSections = computed(() => pagesStore.getPageSections('contact'));

// Get form config from pages.json
const formConfig = computed(() => contactConfig.value?.form || {
  labels: {
    name: 'Name',
    age: 'Age',
    email: 'Email',
...
const handleSubmit = async () => {
  ...
  try {
    ...
    alert(formConfig.value.successMessage)

  } catch (error) {
    console.error('❌ Form submission error:', error)
    alert(formConfig.value.errorMessage)
  } finally {
